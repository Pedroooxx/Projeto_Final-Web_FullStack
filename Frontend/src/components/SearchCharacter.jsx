...
const SearchCharacter = () => {
  ...
  const handleSearch = async (event) => {
    ...
    try {
      const response = await api.get(`/characters/${searchName}`);
      if (response.data) {
      ...
      setMessage("Nome não encontrado.");
      setCharacter(null);
  ...
  return (
    <div className="w-full flex flex-col items-center justify-center font-sans my-12 lg:gap-8 px-8">
      ...
      <div className="p-8 bg-black bg-opacity-90 text-left rounded-3xl w-full max-w-[400px]">
        <h2 className="lg:text-2xl font-bold text-nowrap mb-4 sm:text-lg">
      ...
      {message && (
        <div
          className="text-center py-2 px-4 bg-gray-800 text-white rounded-full mt-4"
